...
class SpecialViewController: UIViewController, UITableViewDelegate, UITableViewDataSource  {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let imgurl = specialListModelArray[indexPath.row].imgurl
    ...
    override func viewDidLoad() {
        ...
        //搜索结果列表
        self.SpecialTableView = UITableView()
        self.SpecialTableView.register(SpecialListCell.self, forCellReuseIdentifier: "SpecialListCell")
        self.SpecialTableView.delegate = self
        self.SpecialTableView.dataSource = self
        self.SpecialTableView.separatorColor = SeparatorColor
        self.SpecialTableView.tableFooterView = UIView()
        self.SpecialTableView.separatorInset = UIEdgeInsets(top: 0, left: 85, bottom: 0, right: 0)
        ...
        //默认【上拉加载】
        self.SpecialTableView.mj_footer = MJRefreshAutoNormalFooter(refreshingTarget: self, refreshingAction: #selector(SpecialViewController.loadMoreData))
    ...
    func reloadData(){
        DispatchQueue.main.async(execute: {
            ...
            self.SpecialTableView.snp.updateConstraints({ (make) in
            ...
            if SpecialModel.noMoreData == false{
                ...
                self.SpecialTableView.mj_footer.endRefreshingWithNoMoreData()
            }
        })
    }
